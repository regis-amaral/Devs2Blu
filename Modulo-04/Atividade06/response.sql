...
SELECT max(p.precovenda) AS "Maior preço de venda dos produtos" FROM produto p;
--8. Encontre o o maior valor de uma determinada comanda;
SELECT max(comanda.valor) AS "Comanda com maior valor" FROM comanda;
--9. Calcule a média de valor total das comanda;
SELECT avg(c.valor) AS "Média dos valores das comandas" FROM comanda c;
